...
static struct QMState const l_msm_top_s = {(struct QMState*)0, Q_STATE_CAST(0), Q_ACTION_CAST(0),
                                           Q_ACTION_CAST(0), Q_ACTION_CAST(0)};
...
#endif
{
     ...
     * be taken yet.
     */
    Q_REQUIRE_ID(200, (me->vptr != (struct QHsmVtable*)0) && (me->temp.fun != Q_STATE_CAST(0)) &&
...
#endif
{
    ...
    me->state.obj = (r >= (QState)Q_RET_TRAN) ? me->temp.tatbl->target : tatbl->target;
    return r;
}
    ...
    while (s != ts) {
        ...
        if (s->exitAction != Q_ACTION_CAST(0)) {
            QS_CRIT_STAT_
                ...
                (void)
                (*s->exitAction)(me); /* execute the exit action */
