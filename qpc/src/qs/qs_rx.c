...
/* compile only if Q_SPY is defined */
#ifdef Q_SPY
...
void QS_queryCurrObj(uint8_t obj_kind)
{
    if (QS_rxPriv_.currObj[obj_kind] != (void*)0) {
        ...
        switch (obj_kind) {
            ...
            case AO_OBJ:
                QS_FUN_PRE_((*((QHsm*)QS_rxPriv_.currObj[obj_kind])->vptr->getStateHandler)(
                    ((QHsm*)QS_rxPriv_.currObj[obj_kind])));
                break;
            case MP_OBJ:
                QS_MPC_PRE_(((QMPool*)QS_rxPriv_.currObj[obj_kind])->nFree);
                QS_MPC_PRE_(((QMPool*)QS_rxPriv_.currObj[obj_kind])->nMin);
                break;
            case EQ_OBJ:
                QS_EQC_PRE_(((QEQueue*)QS_rxPriv_.currObj[obj_kind])->nFree);
                QS_EQC_PRE_(((QEQueue*)QS_rxPriv_.currObj[obj_kind])->nMin);
                break;
            case TE_OBJ:
                QS_OBJ_PRE_(((QTimeEvt*)QS_rxPriv_.currObj[obj_kind])->act);
                QS_TEC_PRE_(((QTimeEvt*)QS_rxPriv_.currObj[obj_kind])->ctr);
                QS_TEC_PRE_(((QTimeEvt*)QS_rxPriv_.currObj[obj_kind])->interval);
                QS_SIG_PRE_(((QTimeEvt*)QS_rxPriv_.currObj[obj_kind])->super.sig);
                QS_U8_PRE_(((QTimeEvt*)QS_rxPriv_.currObj[obj_kind])->super.refCtr_);
                break;
            default:
                break;
        }
    ...
    switch (l_rx.state) {
        ...
        case WAIT4_REC: {
            switch (b) {
                ...
                case QS_RX_FILL:
                    l_rx.var.poke.fill = ((b == (uint8_t)QS_RX_FILL) ? 1U : 0U);
                    if (QS_rxPriv_.currObj[AP_OBJ] != (void*)0) {
                        l_rx.var.poke.offs = 0U;
                        l_rx.var.poke.idx = 0U;
                        QS_RX_TRAN_(WAIT4_POKE_OFFS);
                    } else {
                        QS_rxReportError_((l_rx.var.poke.fill != 0U) ? (uint8_t)QS_RX_FILL
                                                                     : (uint8_t)QS_RX_POKE);
                        QS_RX_TRAN_(ERROR_STATE);
                    }
                    break;
                ...
                case QS_RX_TEST_PROBE:
                    if (l_testData.tpNum <
                        (uint8_t)(sizeof(l_testData.tpBuf) / sizeof(l_testData.tpBuf[0]))) {
                        l_rx.var.tp.data = 0U;
                        l_rx.var.tp.idx = 0U;
        ...
        case WAIT4_POKE_NUM: {
            if (b > 0U) {
                ...
                l_rx.var.poke.idx = 0U;
                QS_RX_TRAN_((l_rx.var.poke.fill != 0U) ? WAIT4_FILL_DATA : WAIT4_POKE_DATA);
            } else {
                QS_rxReportError_((l_rx.var.poke.fill != 0U) ? (uint8_t)QS_RX_FILL
                                                             : (uint8_t)QS_RX_POKE);
                QS_RX_TRAN_(ERROR_STATE);
        ...
        case WAIT4_EVT_LEN: {
            ...
            if (l_rx.var.evt.idx == (8U * 2U)) {
                if ((l_rx.var.evt.len + sizeof(QEvt)) <= QF_poolGetMaxBlockSize()) {
                    /* report Ack before generating any other QS records */
                    QS_rxReportAck_(QS_RX_EVENT);

                    l_rx.var.evt.e =
                        QF_newX_(((uint_fast16_t)l_rx.var.evt.len + sizeof(QEvt)), 0U, /* margin */
...
static void QS_rxHandleGoodFrame_(uint8_t state)
{
    ...
    switch (state) {
        ...
        case WAIT4_CMD_FRAME: {
            ...
            QS_onCommand(l_rx.var.cmd.cmdId, l_rx.var.cmd.param1, l_rx.var.cmd.param2,
                         l_rx.var.cmd.param3);
#ifdef Q_UTEST
        ...
        case WAIT4_PEEK_FRAME: {
            ...
            ptr = ((uint8_t*)QS_rxPriv_.currObj[AP_OBJ] + l_rx.var.peek.offs);
            QS_TIME_PRE_();                  /* timestamp */
        ...
        case WAIT4_FILL_FRAME: {
            ...
            ptr = ((uint8_t*)QS_rxPriv_.currObj[AP_OBJ] + l_rx.var.poke.offs);
            ...
            if (i < (uint8_t)MAX_OBJ) {
                if (l_rx.var.obj.recId == (uint8_t)QS_RX_CURR_OBJ) {
                    ...
                    if (l_rx.var.obj.addr != 0U) {
                        int_fast16_t filter = (int_fast16_t)((QActive*)l_rx.var.obj.addr)->prio;
                        QS_locFilter_((i == 0) ? filter : -filter);
        ...
        case WAIT4_EVT_FRAME: {
            ...
            if (l_rx.var.evt.prio == 0U) { /* publish */
                ...
                if (QACTIVE_POST_X(QF_active_[l_rx.var.evt.prio], l_rx.var.evt.e, 0U, /* margin */
                                   &QS_rxPriv_) == false) {
                ...
                if (QS_rxPriv_.currObj[SM_OBJ] != (void*)0) {
                    ...
                    QHSM_DISPATCH((QHsm*)QS_rxPriv_.currObj[SM_OBJ], l_rx.var.evt.e, 0U);
                ...
                if (QS_rxPriv_.currObj[SM_OBJ] != (void*)0) {
                    ...
                    QHSM_INIT((QHsm*)QS_rxPriv_.currObj[SM_OBJ], l_rx.var.evt.e, 0U);
                    i = 0x01U; /* success status, recycle needed */
                } else {
                    i = 0x81U; /* failure status, recycle needed */
                }
                ...
                if (QS_rxPriv_.currObj[AO_OBJ] != (void*)0) {
                    if (QACTIVE_POST_X((QActive*)QS_rxPriv_.currObj[AO_OBJ], l_rx.var.evt.e,
                                       0U, /* margin */
                                       &QS_rxPriv_) == false) {
                        /* failed QACTIVE_POST() recycles the event */
                        i = 0x80U; /* failure status, no recycle */
...
static void QS_rxPoke_(void)
{
    uint8_t* ptr = ((uint8_t*)QS_rxPriv_.currObj[AP_OBJ] + l_rx.var.poke.offs);
    switch (l_rx.var.poke.size) {
        case 1:
 ...
 * from the Host (running qutest). For any given API, the function returns
 * the Test-Probe data in the same order as it was received from the Host.
 * If there is no Test-Probe for a given API, or no more Test-Probes for
