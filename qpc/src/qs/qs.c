...
void QS_glbFilter_(int_fast16_t const filter)
{
    ...
    switch (rec) {
        ...
        default:
            ...
            if (isRemove) {
                QS_priv_.glbFilter[rec >> 3U] &= (uint8_t)(~(1U << (rec & 7U)) & 0xFFU);
            } else {
                QS_priv_.glbFilter[rec >> 3U] |= (1U << (rec & 7U));
                /* never turn the last 3 records on (0x7D, 0x7E, 0x7F) */
                QS_priv_.glbFilter[15] &= 0x1FU;
            }
            break;
...
void QS_locFilter_(int_fast16_t const filter)
{
    ...
    switch (qs_id) {
        ...
        default:
            if (qs_id < 0x7FU) {
                if (isRemove) {
                    QS_priv_.locFilter[qs_id >> 3U] &= (uint8_t)(~(1U << (qs_id & 7U)) & 0xFFU);
                } else {
                    QS_priv_.locFilter[qs_id >> 3U] |= (1U << (qs_id & 7U));
                }
            } else {
...
void QS_target_info_pre_(uint8_t isReset)
{
    ...
    QS_U8_PRE_((10U * (uint8_t)(TIME[6] - ZERO)) + (uint8_t)(TIME[7] - ZERO));
    QS_U8_PRE_((10U * (uint8_t)(TIME[3] - ZERO)) + (uint8_t)(TIME[4] - ZERO));
    if (Q_BUILD_TIME[0] == ' ') {
        QS_U8_PRE_(TIME[1] - ZERO);
    } else {
        QS_U8_PRE_((10U * (uint8_t)(TIME[0] - ZERO)) + (uint8_t)(TIME[1] - ZERO));
    }
    ...
    if (Q_BUILD_DATE[4] == ' ') {
        ...
        QS_U8_PRE_((10U * (uint8_t)(DATE[4] - ZERO)) + (uint8_t)(DATE[5] - ZERO));
    }
    ...
    QS_U8_PRE_((10U * (uint8_t)(DATE[9] - ZERO)) + (uint8_t)(DATE[10] - ZERO));
    QS_endRec_();
}
